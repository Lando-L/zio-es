...
object InMemoryStorageSpec extends DefaultRunnableSpec:
  ...
  def spec = suite("InMemoryStorageSpec")(
    testM("persist correctly appends an event to the storage") {      
      for
        ref <- Ref.make[Map[PersitenceId, Queue[Event]]](Map.empty)
        ...
        events <- ref.get
      yield
        assert(events.get(id))(isSome(equalTo(Queue(Event.Increment, Event.Increment, Event.Decrement))))
    },

    testM("retrieve correcly replays the events from the storage") {
      for
        ref <- Ref.make[Map[PersitenceId, Queue[Event]]](Map(id -> Queue(Event.Increment, Event.Increment, Event.Decrement)))
        journal = InMemoryStorage(ref)
        events <- journal.retrieve(id).run(Sink.collectAll)
      yield
        assert(events)(equalTo(Chunk(Event.Increment, Event.Increment, Event.Decrement)))
    },
    ...
    testM("retrieve fails to replay events if id is not found") {
      ...
      yield
        assert(events)(fails(isSubtype[NoSuchElementException](anything)))
    }
  )
